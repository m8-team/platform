...
type accessBindingRepositoryFake struct {
	bindings []authzentity.AccessBinding
}

func (f accessBindingRepositoryFake) ListByResource(_ context.Context, _ authzentity.ResourceRef) ([]authzentity.AccessBinding, error) {
...
func TestCheckAccessUseCaseExecuteFallsBackToBindingsAndCachesResult(t *testing.T) {
	...
	if result.Decision != authzentity.PermissionDecisionAllow {
...
func TestCheckAccessUseCaseExecuteUsesRuntimeResult(t *testing.T) {
	...
	useCase := NewCheckAccessUseCase(
		...
		authorizationCheckerFake{
			result: model.AccessCheckResult{
				Decision:   authzentity.PermissionDecisionConditional,
	...
	if result.Decision != authzentity.PermissionDecisionConditional {
