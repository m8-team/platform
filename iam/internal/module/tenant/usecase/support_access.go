...
import (
	...
	"github.com/m8platform/platform/iam/internal/module/tenant/port"
...
func (u *SupportAccessUseCase) Grant(ctx context.Context, cmd model.GrantSupportAccessCommand) (model.SupportGrantResult, error) {
	now := u.clock.Now().UTC()
	grant, err := entity.NewSupportGrant(entity.NewSupportGrantParams{
...
func (u *SupportAccessUseCase) List(ctx context.Context, query model.ListSupportGrantsQuery) ([]entity.SupportGrant, string, error) {
