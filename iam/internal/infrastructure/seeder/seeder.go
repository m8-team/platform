...
func (r *Runner) Run(ctx context.Context) (*Report, error) {
	...
	for _, tenant := range data.Tenants {
		item := &identityv1.Tenant{
			...
			Labels:      foundationprotokit.LabelsFromMap(tenant.Labels),
			CreatedAt:   foundationprotokit.Timestamp(now),
			UpdatedAt:   foundationprotokit.Timestamp(now),
		}
		if err := foundationstore.SaveProto(ctx, r.store, ydb.TableTenants, item.GetTenantId(), item.GetTenantId(), item, now); err != nil {
	...
	for _, user := range data.Users {
		...
		item := &identityv1.User{
			...
			Labels:         foundationprotokit.LabelsFromMap(user.Labels),
			CreatedAt:      foundationprotokit.Timestamp(now),
			UpdatedAt:      foundationprotokit.Timestamp(now),
			KeycloakUserId: user.KeycloakUserID,
		}
		if err := foundationstore.SaveProto(ctx, r.store, ydb.TableUsers, item.GetUserId(), item.GetTenantId(), item, now); err != nil {
			return nil, fmt.Errorf("user %s: %w", user.UserID, err)
		}
		report.Users++
	}
	...
	for _, membership := range data.Memberships {
		...
		if err := foundationstore.SaveProto(ctx, r.store, ydb.TableMemberships, item.GetMembershipId(), item.GetTenantId(), item, now); err != nil {
			return nil, fmt.Errorf("membership %s: %w", membership.MembershipID, err)
		}
		report.Memberships++
	}
	...
	for _, group := range data.Groups {
		item := &identityv1.Group{
			...
			Labels:      foundationprotokit.LabelsFromMap(group.Labels),
			CreatedAt:   foundationprotokit.Timestamp(now),
			UpdatedAt:   foundationprotokit.Timestamp(now),
		}
		if err := foundationstore.SaveProto(ctx, r.store, ydb.TableGroups, item.GetGroupId(), item.GetTenantId(), item, now); err != nil {
	...
	for _, member := range data.GroupMembers {
		...
		if err := foundationstore.SaveProto(ctx, r.store, ydb.TableGroupMembers, memberID, tenantID, item, now); err != nil {
	...
	for _, account := range data.ServiceAccounts {
		...
		if err := foundationstore.SaveProto(ctx, r.store, ydb.TableServiceAccounts, item.GetServiceAccountId(), item.GetTenantId(), item, now); err != nil {
	...
	for _, client := range data.OAuthClients {
		...
		if err := foundationstore.SaveProto(ctx, r.store, ydb.TableOAuthClients, item.GetOauthClientId(), item.GetTenantId(), item, now); err != nil {
			return nil, fmt.Errorf("oauth client %s: %w", client.OAuthClientID, err)
		}
		report.OAuthClients++
	}
	...
	for _, binding := range data.Bindings {
		...
		item := &authzv1.AccessBinding{
			...
			Labels:    foundationprotokit.LabelsFromMap(binding.Labels),
			CreatedAt: foundationprotokit.Timestamp(now),
		}
		if err := foundationstore.SaveProto(ctx, r.store, ydb.TableBindingOperations, item.GetBindingId(), item.GetResource().GetTenantId(), item, now); err != nil {
