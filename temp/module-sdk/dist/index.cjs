...
__export(index_exports, {
  ...
  buildPathFromRegistry: () => buildPathFromRegistry,
  compact: () => compact,
  createRouterRuntime: () => createRouterRuntime,
  createScopeRuntime: () => createScopeRuntime,
  createValidationResult: () => createValidationResult,
  defineModule: () => defineModule,
  defineModuleManifest: () => defineModuleManifest,
  definePlatformRegistry: () => definePlatformRegistry,
  defineRemoteModule: () => defineRemoteModule,
  errorIssue: () => errorIssue,
  filterNavigationItems: () => filterNavigationItems,
  findInstalledModule: () => findInstalledModule,
  findMountPoint: () => findMountPoint,
  findScope: () => findScope,
  findSlot: () => findSlot,
  interpolatePathTemplate: () => interpolatePathTemplate,
  isModuleVisible: () => isModuleVisible,
  isNavigationItemVisible: () => isNavigationItemVisible,
  joinPaths: () => joinPaths,
  normalizePath: () => normalizePath,
  stripTrailingSlash: () => stripTrailingSlash,
  unique: () => unique,
  validateModuleManifest: () => validateModuleManifest,
  validatePlatformRegistry: () => validatePlatformRegistry,
...
function findInstalledModule(registry, moduleId) {
  return registry.modules.find((module2) => module2.id === moduleId);
}
...
function defineModule(module2) {
  return {
    ...module2,
    getManifest: () => module2.manifest
  };
}
...
function validatePlatformRegistry(registry) {
  ...
  registry.modules.forEach((module2, index) => {
    if (!module2.id) {
      issues.push(errorIssue("module.id.required", "Installed module id is required.", `modules.${index}.id`));
    }
    if (moduleIds.has(module2.id)) {
      issues.push(errorIssue("module.id.duplicate", `Duplicate installed module id: ${module2.id}`, `modules.${index}.id`));
    }
    moduleIds.add(module2.id);
  });
