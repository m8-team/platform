...
Новый ADR всегда начинается со статуса 🟡 **Proposed**. Шаблон не утверждает решение. Используйте [правила ADR](../../adr/README.md). Готовый ADR размещается в `docs/adr/{category}/{number}-{short-name}.md`; после переноса пересчитайте относительные ссылки.
