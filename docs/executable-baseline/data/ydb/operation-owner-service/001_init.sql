-- M8 YDB schema baseline for operation owner service.
PRAGMA TablePathPrefix("/local/m8");

    -- Common operational tables for operation owner service.
    CREATE TABLE `operation owner service/outbox` (
    ...
    CREATE TABLE `operation owner service/inbox` (
    ...
    CREATE TABLE `operation owner service/idempotency_keys` (
        scope Utf8 NOT NULL,
    ...
    CREATE TABLE `operation owner service/operations` (
    ...
    -- DATA-OPS-001: Operation; classification=internal; retention=90d-7y by operation type.
    CREATE TABLE `operation owner service/operations` (
        id Utf8 NOT NULL,
        parent_id Utf8,
        project_id Utf8,
        ...
        PRIMARY KEY (id),
        INDEX idx_operations_parent GLOBAL ON (parent_id, created_at),
        INDEX idx_operations_project GLOBAL ON (project_id, created_at),
        INDEX idx_operations_status GLOBAL ON (status, updated_at)
    ) WITH (
        AUTO_PARTITIONING_BY_SIZE = ENABLED,
        AUTO_PARTITIONING_BY_LOAD = ENABLED
    );
