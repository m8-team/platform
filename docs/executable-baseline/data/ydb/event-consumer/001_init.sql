-- M8 YDB schema baseline for event consumer.
PRAGMA TablePathPrefix("/local/m8");

    -- Common operational tables for event consumer.
    CREATE TABLE `event consumer/outbox` (
    ...
    CREATE TABLE `event consumer/inbox` (
    ...
    CREATE TABLE `event consumer/idempotency_keys` (
        scope Utf8 NOT NULL,
    ...
    CREATE TABLE `event consumer/operations` (
    ...
    -- DATA-COMMON-002: InboxRecord; classification=internal; retention=dedup window.
    CREATE TABLE `event consumer/inbox_records` (
        id Utf8 NOT NULL,
        parent_id Utf8,
        project_id Utf8,
        status Utf8 NOT NULL,
        ...
        PRIMARY KEY (id),
        INDEX idx_inbox_records_parent GLOBAL ON (parent_id, created_at),
        INDEX idx_inbox_records_project GLOBAL ON (project_id, created_at),
        INDEX idx_inbox_records_status GLOBAL ON (status, updated_at)
    ) WITH (
        AUTO_PARTITIONING_BY_SIZE = ENABLED,
        AUTO_PARTITIONING_BY_LOAD = ENABLED
    );
