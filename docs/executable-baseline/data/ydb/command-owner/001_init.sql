-- M8 YDB schema baseline for command owner.
PRAGMA TablePathPrefix("/local/m8");

    -- Common operational tables for command owner.
    CREATE TABLE `command owner/outbox` (
    ...
    CREATE TABLE `command owner/inbox` (
    ...
    CREATE TABLE `command owner/idempotency_keys` (
        scope Utf8 NOT NULL,
    ...
    CREATE TABLE `command owner/operations` (
    ...
    -- DATA-COMMON-003: IdempotencyRecord; classification=internal; retention=documented idempotency window.
    CREATE TABLE `command owner/idempotency_records` (
        id Utf8 NOT NULL,
        parent_id Utf8,
        project_id Utf8,
        status Utf8 NOT NULL,
        ...
        PRIMARY KEY (id),
        INDEX idx_idempotency_records_parent GLOBAL ON (parent_id, created_at),
        INDEX idx_idempotency_records_project GLOBAL ON (project_id, created_at),
        INDEX idx_idempotency_records_status GLOBAL ON (status, updated_at)
    ) WITH (
        AUTO_PARTITIONING_BY_SIZE = ENABLED,
        AUTO_PARTITIONING_BY_LOAD = ENABLED
    );
