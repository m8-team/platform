...
# Resource Manager API, gRPC: ServiceService.DeleteService

Soft-deletes a service by its UUID.
...
| --- | --- | --- |
| id | string | Required. Stable unique identifier of the service to delete.<br/>The value must be a valid UUID string. |
| version | int64 | Optional. The version of the service.<br/>If provided, the value must exactly match the current server-side version. |
| allow_missing | bool | Optional. If true, the request succeeds even if the service does not<br/>exist or has already been soft-deleted. |
