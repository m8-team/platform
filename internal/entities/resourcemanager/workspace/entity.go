...
import (
	...
	"github.com/m8platform/platform/internal/entities/resourcemanager/shared"
)
...
func New(params CreateParams) (Entity, error) {
	...
	if _, err := uuid.Parse(params.OrganizationID); err != nil {
		return Entity{}, ErrInvalidParentID
	}
...
func (e *Entity) Update(mask []string, params UpdateParams, now time.Time, nextETag string) error {
	...
	if params.OrganizationID != "" && params.OrganizationID != e.OrganizationID {
