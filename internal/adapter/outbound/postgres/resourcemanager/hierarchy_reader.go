...
func (r HierarchyReader) GetOrganizationNode(_ context.Context, id string) (port.HierarchyNode, error) {
	r.Store.mu.RLock()
	defer r.Store.mu.RUnlock()

	entity, ok := r.Store.organizations[id]
	if !ok {
		return port.HierarchyNode{ID: id}, nil
	}
	return port.HierarchyNode{
		ID:      id,
		Exists:  true,
...
func (r HierarchyReader) GetWorkspaceNode(_ context.Context, id string) (port.HierarchyNode, error) {
	r.Store.mu.RLock()
	defer r.Store.mu.RUnlock()
