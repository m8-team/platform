...
func TestOrganizationServiceCreateAndGet(t *testing.T) {
	...
	created, err := h.service.Create(context.Background(), organizationapp.CreateOrganization{
		Name:        "Acme",
	...
	got, err := h.service.Get(context.Background(), organizationapp.GetOrganization{ID: created.ID()})
	if err != nil {
...
func TestOrganizationServiceListUsesSignedKeysetTokens(t *testing.T) {
	...
	for _, item := range []struct {
		...
		if _, err := h.service.Create(context.Background(), organizationapp.CreateOrganization{
			Name:   item.name,
			Labels: item.labels,
		}); err != nil {
	...
	first, err := h.service.List(context.Background(), organizationapp.ListOrganizations{
		PageSize: 2,
		OrderBy:  "name asc",
	})
	if err != nil {
	...
	if _, err := h.service.Create(context.Background(), organizationapp.CreateOrganization{Name: "Aardvark"}); err != nil {
		t.Fatalf("Create(Aardvark) error = %v", err)
	}
	second, err := h.service.List(context.Background(), organizationapp.ListOrganizations{
		PageSize:  2,
	...
	t.Run("tampered signature", func(t *testing.T) {
		_, err := h.service.List(context.Background(), organizationapp.ListOrganizations{
			PageSize:  2,
			PageToken: tamperPageToken(t, first.NextPageToken),
			OrderBy:   "name asc",
		})
		if !errors.Is(err, organizationapp.ErrInvalidOrganizationPageToken) {
			t.Fatalf("List() error = %v, want %v", err, organizationapp.ErrInvalidOrganizationPageToken)
		}
	})

	t.Run("different signing key", func(t *testing.T) {
		other := newService(t, h.repository, h.authorizer, h.clock, h.ids, h.children, organizationapp.OrganizationServiceConfig{
			SoftDeleteRetention: 48 * time.Hour,
			PageTokenKey:        []byte("abcdef0123456789abcdef0123456789"),
		})
		_, err := other.List(context.Background(), organizationapp.ListOrganizations{
			PageSize:  2,
			PageToken: first.NextPageToken,
			OrderBy:   "name asc",
		})
	...
	for name, changedRequest := range map[string]organizationapp.ListOrganizations{
		"page size": {
...
func TestOrganizationServiceListFiltersAndDeletedVisibility(t *testing.T) {
	...
	h := newHarness(t)
	prod, err := h.service.Create(context.Background(), organizationapp.CreateOrganization{
		Name:   "Production",
	...
	if _, err := h.service.Delete(context.Background(), organizationapp.DeleteOrganization{ID: prod.ID()}); err != nil {
		t.Fatalf("Delete(Production) error = %v", err)
	}

	visible, err := h.service.List(context.Background(), organizationapp.ListOrganizations{})
	if err != nil {
		t.Fatalf("List(default) error = %v", err)
	}
	assertOrganizationNames(t, visible.Organizations, "Development")

	deleted, err := h.service.List(context.Background(), organizationapp.ListOrganizations{
	...
	_, err = h.service.List(context.Background(), organizationapp.ListOrganizations{Filter: `description == "unsupported"`})
	if !errors.Is(err, organizationapp.ErrInvalidOrganizationFilter) {
		t.Fatalf("List(unsupported filter) error = %v, want %v", err, organizationapp.ErrInvalidOrganizationFilter)
	}
	_, err = h.service.List(context.Background(), organizationapp.ListOrganizations{OrderBy: "state asc"})
	if !errors.Is(err, organizationapp.ErrInvalidOrganizationOrderBy) {
		t.Fatalf("List(unsupported order) error = %v, want %v", err, organizationapp.ErrInvalidOrganizationOrderBy)
	}
}
...
func TestOrganizationServiceUpdateUsesPointersAndVersionPreconditions(t *testing.T) {
	...
	labels := map[string]string{}
	updated, err := h.service.Update(context.Background(), organizationapp.UpdateOrganization{
		ID:              created.ID(),
	...
	_, err = h.service.Update(context.Background(), organizationapp.UpdateOrganization{
		ID:              created.ID(),
		ExpectedVersion: types.InitialVersion,
	...
	_, err = h.service.Update(context.Background(), organizationapp.UpdateOrganization{ID: created.ID()})
...
func TestOrganizationServiceUpdateSurfacesRepositoryCASConflict(t *testing.T) {
	...
	created, err := service.Create(context.Background(), organizationapp.CreateOrganization{Name: "Original"})
	if err != nil {
		t.Fatalf("Create() error = %v", err)
	}
	...
	_, err = service.Update(context.Background(), organizationapp.UpdateOrganization{
		ID:          created.ID(),
...
func TestOrganizationServiceDeleteAllowMissingChildBlockerAndPurgeTimes(t *testing.T) {
	...
	deleted, err := h.service.Delete(context.Background(), organizationapp.DeleteOrganization{
		ID:           missingID,
		AllowMissing: true,
	})
	...
	_, err = h.service.Delete(context.Background(), organizationapp.DeleteOrganization{ID: missingID})
	if !errors.Is(err, ports.ErrOrganizationNotFound) {
		t.Fatalf("Delete(missing) error = %v, want %v", err, ports.ErrOrganizationNotFound)
	}

	created, err := h.service.Create(context.Background(), organizationapp.CreateOrganization{Name: "Has workspaces"})
	if err != nil {
		t.Fatalf("Create() error = %v", err)
	}
	h.children.hasNonDeleted = true
	h.clock.now = testStartTime.Add(time.Hour)
	_, err = h.service.Delete(context.Background(), organizationapp.DeleteOrganization{
		ID:              created.ID(),
		ExpectedVersion: types.InitialVersion,
	})
	if !errors.Is(err, organizationapp.ErrOrganizationHasWorkspaces) {
		t.Fatalf("Delete(with children) error = %v, want %v", err, organizationapp.ErrOrganizationHasWorkspaces)
	}
	stored, err := h.repository.Get(context.Background(), created.ID())
	if err != nil {
		t.Fatalf("repository.Get() error = %v", err)
	...
	unchanged, err := h.service.Delete(context.Background(), organizationapp.DeleteOrganization{
		ID:           created.ID(),
		AllowMissing: true,
	})
	if err != nil {
	...
	_, err = h.service.Delete(context.Background(), organizationapp.DeleteOrganization{ID: created.ID()})
...
func TestOrganizationServiceUndeleteRestoresRetainedOrganization(t *testing.T) {
	...
	h := newHarness(t)
	created, err := h.service.Create(context.Background(), organizationapp.CreateOrganization{Name: "Recoverable"})
	if err != nil {
		t.Fatalf("Create() error = %v", err)
	}
	h.clock.now = testStartTime.Add(time.Hour)
	deleted, err := h.service.Delete(context.Background(), organizationapp.DeleteOrganization{ID: created.ID()})
	if err != nil {
		t.Fatalf("Delete() error = %v", err)
	}
	...
	restored, err := h.service.Undelete(context.Background(), organizationapp.UndeleteOrganization{ID: created.ID()})
	if err != nil {
	...
	_, err = h.service.Undelete(context.Background(), organizationapp.UndeleteOrganization{ID: created.ID()})
	...
	second, err := h.service.Create(context.Background(), organizationapp.CreateOrganization{Name: "Expired"})
	if err != nil {
		t.Fatalf("Create(Expired) error = %v", err)
	}
	h.clock.now = testStartTime.Add(3 * time.Hour)
	expired, err := h.service.Delete(context.Background(), organizationapp.DeleteOrganization{ID: second.ID()})
	if err != nil {
		t.Fatalf("Delete(Expired) error = %v", err)
	}
	h.clock.now = *expired.PurgeTime()
	_, err = h.service.Undelete(context.Background(), organizationapp.UndeleteOrganization{ID: second.ID()})
...
func TestOrganizationServiceAuthorizationDeniesEveryActionBeforeWork(t *testing.T) {
	...
	tests := []struct {
		...
		id     organization.ID
		call   func(*organizationapp.OrganizationService) error
	}{
		{
			name: "create", action: ports.ActionCreateOrganization,
			call: func(service *organizationapp.OrganizationService) error {
				_, err := service.Create(context.Background(), organizationapp.CreateOrganization{Name: "Denied"})
				return err
			},
		},
		{
			...
			call: func(service *organizationapp.OrganizationService) error {
				_, err := service.Get(context.Background(), organizationapp.GetOrganization{ID: id})
				return err
			},
		},
		{
			...
			call: func(service *organizationapp.OrganizationService) error {
				_, err := service.List(context.Background(), organizationapp.ListOrganizations{})
				return err
			},
		},
		{
			...
			call: func(service *organizationapp.OrganizationService) error {
				_, err := service.Update(context.Background(), organizationapp.UpdateOrganization{ID: id, Name: ptr("Denied")})
				return err
			},
		},
		{
			...
			call: func(service *organizationapp.OrganizationService) error {
				_, err := service.Delete(context.Background(), organizationapp.DeleteOrganization{ID: id, AllowMissing: true})
				return err
			},
		},
		{
			...
			call: func(service *organizationapp.OrganizationService) error {
				_, err := service.Undelete(context.Background(), organizationapp.UndeleteOrganization{ID: id})
				return err
			},
		},
	}
...
func TestOrganizationServiceAuthorizesAllActionsWithCorrectScope(t *testing.T) {
	...
	h := newHarness(t)
	created, err := h.service.Create(context.Background(), organizationapp.CreateOrganization{Name: "Lifecycle"})
	if err != nil {
		t.Fatalf("Create() error = %v", err)
	}
	if _, err := h.service.Get(context.Background(), organizationapp.GetOrganization{ID: created.ID()}); err != nil {
		t.Fatalf("Get() error = %v", err)
	}
	if _, err := h.service.List(context.Background(), organizationapp.ListOrganizations{}); err != nil {
		t.Fatalf("List() error = %v", err)
	}
	h.clock.now = testStartTime.Add(time.Minute)
	if _, err := h.service.Update(context.Background(), organizationapp.UpdateOrganization{
		ID: created.ID(), Name: ptr("Updated"),
	}); err != nil {
		t.Fatalf("Update() error = %v", err)
	}
	h.clock.now = testStartTime.Add(2 * time.Minute)
	if _, err := h.service.Delete(context.Background(), organizationapp.DeleteOrganization{ID: created.ID()}); err != nil {
		t.Fatalf("Delete() error = %v", err)
	}
	h.clock.now = testStartTime.Add(3 * time.Minute)
	if _, err := h.service.Undelete(context.Background(), organizationapp.UndeleteOrganization{ID: created.ID()}); err != nil {
		t.Fatalf("Undelete() error = %v", err)
	}
...
type harness struct {
	service    *organizationapp.OrganizationService
...
func defaultServiceConfig() organizationapp.OrganizationServiceConfig {
	return organizationapp.OrganizationServiceConfig{
		SoftDeleteRetention: 48 * time.Hour,
...
func newService(
	...
	config organizationapp.OrganizationServiceConfig,
) *organizationapp.OrganizationService {
	t.Helper()

	service, err := organizationapp.NewOrganizationService(repository, authorizer, clock, ids, children, config)
	if err != nil {
