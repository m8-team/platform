...
func TestWorkspaceServiceCreateUpdateDeleteAndUndelete(t *testing.T) {
	...
	created, err := h.service.Create(context.Background(), workspaceapp.CreateWorkspace{
	...
	name := "Primary"
	updated, err := h.service.Update(context.Background(), workspaceapp.UpdateWorkspace{
		ID:              created.ID(),
	...
	if _, err := h.service.Update(context.Background(), workspaceapp.UpdateWorkspace{
	...
	h.clock.now = h.clock.now.Add(time.Minute)
	deleted, err := h.service.Delete(context.Background(), workspaceapp.DeleteWorkspace{
	...
	h.clock.now = h.clock.now.Add(time.Minute)
	restored, err := h.service.Undelete(context.Background(), workspaceapp.UndeleteWorkspace{ID: created.ID()})
	if err != nil {
...
func TestWorkspaceServiceListUsesParentBoundSignedKeysetTokens(t *testing.T) {
	...
	for _, item := range []struct {
		...
		if _, err := h.service.Create(context.Background(), workspaceapp.CreateWorkspace{
	...
	first, err := h.service.List(context.Background(), workspaceapp.ListWorkspaces{
		OrganizationID: h.organizationID,
		PageSize:       2,
		OrderBy:        "name asc",
	...
	page, err := h.service.List(context.Background(), workspaceapp.ListWorkspaces{
	...
	second, err := h.service.List(context.Background(), workspaceapp.ListWorkspaces{
	...
	_, err = h.service.List(context.Background(), workspaceapp.ListWorkspaces{
		OrganizationID: secondOrganization.ID(), PageSize: 1, PageToken: page.NextPageToken, OrderBy: "name asc",
	})
	if !errors.Is(err, workspaceapp.ErrInvalidWorkspacePageToken) {
		t.Fatalf("cross-parent token error = %v, want %v", err, workspaceapp.ErrInvalidWorkspacePageToken)
	}
	_, err = h.service.List(context.Background(), workspaceapp.ListWorkspaces{
		OrganizationID: h.organizationID, PageSize: 1, PageToken: tamperPageToken(t, page.NextPageToken), OrderBy: "name asc",
	})
	if !errors.Is(err, workspaceapp.ErrInvalidWorkspacePageToken) {
		t.Fatalf("tampered token error = %v, want %v", err, workspaceapp.ErrInvalidWorkspacePageToken)
	}
	_, err = h.service.List(context.Background(), workspaceapp.ListWorkspaces{
		OrganizationID: h.organizationID, PageSize: 2, PageToken: page.NextPageToken, OrderBy: "name asc",
	})
	if !errors.Is(err, workspaceapp.ErrInvalidWorkspacePageToken) {
		t.Fatalf("changed page-size token error = %v, want %v", err, workspaceapp.ErrInvalidWorkspacePageToken)
	}
}
...
func TestWorkspaceServiceRejectsInvalidGeneratedID(t *testing.T) {
	h := newWorkspaceHarness(t)
	h.ids.ids = nil
	_, err := h.service.Create(context.Background(), workspaceapp.CreateWorkspace{OrganizationID: h.organizationID})
	if !errors.Is(err, workspaceapp.ErrGeneratedWorkspaceID) {
		t.Fatalf("Create() error = %v, want %v", err, workspaceapp.ErrGeneratedWorkspaceID)
	}
}
...
func TestWorkspaceServiceAuthorizesBeforeMissingDeleteLookup(t *testing.T) {
	...
	_, err := service.Delete(context.Background(), workspaceapp.DeleteWorkspace{
...
func TestWorkspacePreventsDeletingParentAndRestoringUnderDeletedParent(t *testing.T) {
	h := newWorkspaceHarness(t)
	created, err := h.service.Create(context.Background(), workspaceapp.CreateWorkspace{OrganizationID: h.organizationID})
	if err != nil {
		t.Fatal(err)
	}
	...
	if _, err := organizationService.Delete(context.Background(), organizationapp.DeleteOrganization{ID: h.organizationID}); !errors.Is(err, organizationapp.ErrOrganizationHasWorkspaces) {
		t.Fatalf("Delete organization error = %v, want %v", err, organizationapp.ErrOrganizationHasWorkspaces)
	}

	h.clock.now = h.clock.now.Add(time.Minute)
	deleted, err := h.service.Delete(context.Background(), workspaceapp.DeleteWorkspace{ID: created.ID()})
	if err != nil {
		t.Fatal(err)
	}
	h.clock.now = h.clock.now.Add(time.Minute)
	if _, err := organizationService.Delete(context.Background(), organizationapp.DeleteOrganization{ID: h.organizationID}); err != nil {
		t.Fatalf("Delete empty organization error = %v", err)
	}
	h.clock.now = h.clock.now.Add(time.Minute)
	if _, err := h.service.Undelete(context.Background(), workspaceapp.UndeleteWorkspace{ID: deleted.ID()}); !errors.Is(err, organization.ErrOrganizationNotActive) {
...
func TestWorkspaceCreateAndOrganizationDeletePreserveHierarchyInvariant(t *testing.T) {
	for iteration := 0; iteration < 20; iteration++ {
		...
		go func() {
			<-start
			_, _ = workspaceService.Create(context.Background(), workspaceapp.CreateWorkspace{OrganizationID: parent.ID()})
			done <- struct{}{}
		}()
		go func() {
			<-start
			_, _ = organizationService.Delete(context.Background(), organizationapp.DeleteOrganization{ID: parent.ID()})
			done <- struct{}{}
		}()
		close(start)
		<-done
...
type workspaceHarness struct {
	service        *workspaceapp.WorkspaceService
...
func newWorkspaceService(
	...
	t.Helper()
	service, err := workspaceapp.NewWorkspaceService(repository, organizations, authorizer, clock, ids, workspaceapp.WorkspaceServiceConfig{
