...
// ID is the stable, module-owned identifier of a Workspace.
type ID struct {
	value types.ID
}

// NewID creates a new non-zero Workspace identifier.
func NewID() ID {
	return ID{value: types.New()}
}

// ParseID parses a UUID string into a Workspace identifier.
func ParseID(value string) (ID, error) {
	if value == "" {
		return ID{}, ErrEmptyWorkspaceID
	}

	parsed, err := types.Parse(value)
	if err != nil {
		return ID{}, fmt.Errorf("%w: %q: %v", ErrInvalidWorkspaceID, value, err)
	}

	return ID{value: parsed}, nil
}

// MustParseID parses value and panics when it is not a valid Workspace ID.
